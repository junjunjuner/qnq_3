...
from qnq_3.settings import FIELDS_TO_EXPORT
from scrapy import signals
from scrapy.exporters import CsvItemExporter
import time


class Qnq3Pipeline(object):
    def __init__(self):
        # 连接数据库
...
class CSVPipeline(object):
  ...
  def spider_opened(self, spider):
    # file = open('jd取暖器_%s.csv' % (self.printfNow()), 'wb')
    file = open('jdqnq_3items%s.csv' % (self.printfNow()), 'wb')
